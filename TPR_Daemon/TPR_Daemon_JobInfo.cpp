...
TPR_Daemon_JobInfo::TPR_Daemon_JobInfo( const int nJobId )
{
    ...
    {
        m_mapJobStatus[JOB_STATUS_BLOCKED_DEVQ] = L"The driver cannot print the job.";
        m_mapJobStatus[JOB_STATUS_COMPLETE] = L"Job is sent to the printer, but the job may not be printed yet.";
        m_mapJobStatus[JOB_STATUS_DELETED] = L"Job has been deleted.";
        m_mapJobStatus[JOB_STATUS_DELETING] = L"Job is being deleted.";
        m_mapJobStatus[JOB_STATUS_ERROR] = L"An error is associated with the job.";
        m_mapJobStatus[JOB_STATUS_OFFLINE] = L"Printer is offline.";
        m_mapJobStatus[JOB_STATUS_PAPEROUT] = L"Printer is out of paper.";
        m_mapJobStatus[JOB_STATUS_PAUSED] = L"Job is paused.";
        m_mapJobStatus[JOB_STATUS_PRINTED] = L"Job has printed.";
        m_mapJobStatus[JOB_STATUS_PRINTING] = L"Job is printing.";
        m_mapJobStatus[JOB_STATUS_RESTART] = L"Job has been restarted.";
        m_mapJobStatus[JOB_STATUS_SPOOLING] = L"Job is spooling.";
        m_mapJobStatus[JOB_STATUS_USER_INTERVENTION] = L"Printer has an error that requires the user to do something.";
    }
}

std::map<int, LPCTSTR> TPR_Daemon_JobInfo::m_mapJobStatus;
...
LPCTSTR TPR_Daemon_JobInfo::GetUser( void ) const
{
	return m_strUser;
}

LPCTSTR TPR_Daemon_JobInfo::GetMachine( void ) const
{
    return m_strMachine;
}

LPCTSTR TPR_Daemon_JobInfo::GetPort( void ) const
{
    return m_strPort;
}

LPCTSTR TPR_Daemon_JobInfo::GetDocument( void ) const
{
...
LPCTSTR TPR_Daemon_JobInfo::GetStatus( void ) const
{
